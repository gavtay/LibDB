...
ORDER BY BorrowDate;
-- Joining the Borrower and Client table together so that records from both can be used. Then I created a condition that requires the month and date to be March, 2018. The selected results are then ordered by the date they were borrowed in Ascending order for March of 2018.
...
LIMIT 5;
-- Selecting count of each instance that an author ID is used to borrow, Joining Author to book with the AuthorID key, and then joining book to borrower using BookID key. Created a condidition that requires the year of the BorrowDate to be 2017. Then Group by the remaining selects, order by the count of AuthorIDs selected at the beginning in descending order to make sure the records are displayed from highest to lowest, and then set the limit to show only the top 5 records.
...
LIMIT 5;
-- Created another count of AuthorNationality to know the amount of times each nationality was borrowed. Joined Borrower to Book with the BookID key, then joined Book to Author with the AuthorID key. Created a condition that requires the year of the borrowdate to be between the years 2015 to 2017 per the requirements of the problem. Grouped by all selects except for the count select, then ordered by the count of AuthorNationality. Finally limited the records to 5 so it returns the 5 least authors clients borrowed between the years 2015 to 2017.
...
LIMIT 1;
-- Once again, a count was created for each book ID that was borrowed. Borrower and Book were joined using the BookID key. This was based on a condition that required the borrowdate to be between the years 2015-2017. BookTitle was the grouping performed and the count was used to order the records in descending order to show the highest results on top. The records were then limited to 1 to show only the top book.
...
LIMIT 10;
-- Surprise, a count was created for the genres. Borrower was joined to client using ClientID key, then Borrower to Book using the BookID key. A conditional was made that requires the clients date of birth to be between 1970 and 1980. Genre was the grouping and the order was the count of genre in descending order so that the highest amounts of genres recorded would be at the top of the records returned. 
...
LIMIT 5;
-- As you may already assume, a count was created for Book IDs. The table borrower was joined with client using the ClientID key. We specifically looked for records that the borrowdate year was 2017, then grouped by Occupation, and ordered by the bookID count in Desceding order and finally limited the query results to 5 to only show the top 5 occupations.
